...
<div style="padding: 10px 0px;">
    <table>
        <tr>
            ...
            <td><label for="levelSelection">{translate key=Level}:</label></td>
            <td><label for="filterStatusId">{translate key=Groups}:</label></td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <option value="{$level}" {if $selectedLevel eq $level}selected="selected"{/if}>{$level}</option>
                {/foreach}         	
							</select>
            </td>
            <td>
            	<select id="grouppSelection" class="textbox">
              	<option value="0">All</option>
            		{foreach from=$Groupps item=group}
                <option value="{$group['grouppid']}" {if $selectedGroup eq $group['grouppid']}selected="selected"{/if}>{$group['grouppname']}</option>
                {/foreach}         	
							</select>
            </td>
        </tr>
...
{foreach from=$users item=user}
    ...
    <tr class="{$rowCss} editable">
        ...
        <td align="center">{$user->group_name} <a href="#" class="update changeGroups">{translate key='Edit'}</a></td>
        ...
        <td align="center"><a href="#" class="update editProfilePhoto" userid="{$user->Id}">{translate key='Edit'}</a></td>
	{if $Definitions|count > 0}
        <td>
            <div class="editable">
                {*<ul>
        {foreach from=$AttributeList->GetAttributeValues($user->Id) item=attribute}
                        <li style="list-style:none;" class="customAttribute" attributeId="{$attribute->Id()}">
	...
	{/if}
      ...
      <tr>
		<td class="id" style="display: none;"><input type="hidden" class="id" value="{$user->Id}"/></td>
		<td colspan="17" style="display: none;" class="{$rowCss} customAttributes" userId="{$user->Id}">
            ...
            <div style="display: table-row">
                ...
                <div style="display: table-cell;">
                    <ul>
                        ...
                        <li>{textbox name="PHONE" class="textbox" size="40" id="addPhone"}</li>
                    </ul>
                </div>
                <div style="display: table-cell;">
                    <ul>
    ...
    $(document).ready(function ()
    {
        ...
        var userOptions = {
            ...
            filterUrl:'{$smarty.server.SCRIPT_NAME}?{QueryStringKeys::ACCOUNT_STATUS}=',
						filterUrl2:'{$smarty.server.SCRIPT_NAME}?lvlsel=',
						filterUrl3:'{$smarty.server.SCRIPT_NAME}?grpsel=',
            actions:actions,
