...
<table class="list">
	<tr>
		<th class="id">&nbsp;</th>
		<th style="text-align: left;">{translate key='GroupName'}</th>
		<th>{translate key='Actions'}</th>
		<th>{translate key='GroupMembers'}</th>
		<th>{translate key='Permissions'}</th>
        <th>{translate key='freeofcharge'}</th>
        <th>{translate key='coaches'}</th>
        <th>{translate key='guests'}</th>
	</tr>
{foreach from=$groups item=group}
	...
	<tr class="{$rowCss}">
        ...
        <td align="center"><input type="checkbox" class="update-group-details" group-state-type="free_of_charge" group="{$group->Id}" {if in_array($group->Id,$freeGroups)}checked="checked"{/if} name="foc_{$group->Id}" /></td>
        <td align="center"><input type="checkbox" class="update-group-details" group-state-type="coaches" group="{$group->Id}" {if in_array($group->Id,$coachesGroups)}checked="checked"{/if} name="coaches_{$group->Id}" /></td>
        <td align="center"><input type="checkbox" class="update-group-details" group-state-type="guests" group="{$group->Id}" {if in_array($group->Id,$guestsGroups)}checked="checked"{/if} name="guests_{$group->Id}" /></td>
	</tr>
{/foreach}
</table>
...
<div class="admin" style="margin-top:30px">
	...
	<div>
		...
		<form id="addGroupForm" method="post">
			{translate key=Name}<br/> <input type="text" class="textbox required" {formname key=GROUP_NAME} />
