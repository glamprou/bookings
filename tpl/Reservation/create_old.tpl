		...
		<ul class="no-style">
            ...
            <li>
				...
				<div id="repeatDiv">
					...
					<select id="chooseCallOptions" {formname key=CHOOSEN_CALL_ID} class="pulldown" style="width:250px" >
						{if !$onlyone}<option value="-1">{translate key="chooseNoCall"}</option>{/if}
...
<script type="text/javascript">
	...
	if($('#chooseCallOptions').val()!=undefined){
		if($('#chooseCallOptions').val()!=-1){
			$('#psiattribute\\[1\\]').val('true');
		}
		else{
			$('#psiattribute\\[1\\]').val('false');
		}
		$('#chooseCallOptions').change(function(){
			if($('#chooseCallOptions').val()!=-1){
				$('#psiattribute\\[1\\]').val('true');
			}
			else{
