function Reservation(opts) {
	...
	var WireUpSaveDialog = function() {
        ...
        $(document).on('click', '#exceededQuotaAdminAccept', function() {
            $('#reservationForm').append('<input type="hidden" name="exceededQuotaAdminAck" value="true" />').submit();
        });
        $(document).on('click', '#exceededQuotaAdminCancel', function() {
            CloseSaveDialog();
        });
	};
	...
	{
		//$('#ParticipantsNeeded').css('visibility','hidden');
	...
	participation.removeParticipant = function(userId)
	{
		...
		if(participation.addedUsers.length==1){
			//$('#ParticipantsNeeded').css('visibility','visible');
		}
	};
    ...
    $('#chooseACall').change(function(){
        $('.remove','#participantList').click();
        if($(this).val()!='-1'){
            participation.addParticipant($('option:selected','#chooseACall').attr('oppname'),$('option:selected','#chooseACall').attr('oppid'));
            $('#participationOverLayer').show();
        }
        else{
            $('#participationOverLayer').hide();
        }
    });
}
