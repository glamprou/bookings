function GenerateReports(reportOptions) {
	...
	GenerateReports.prototype.init = function () {
		$('#selectDiv input').click(function () {
            ...
            else if($(this).attr('id') == 'results_training'){
                $('#emptyDiv').show();
                $('#groupby_user_training').click();
...
$(document).ready(function(){
    $("input[name=REPORT_GROUPBY]").click(function(){
        $('#training-search').attr('disabled', 'disabled');
        if($(this).attr('id') == 'groupby_user_training'){
            $('#training-search').removeAttr('disabled');
        }
        else if($(this).val()==="USER"){
            $("#zeroResUsersContainer").show();
        }
        else{
            $("#zeroResUsersContainer").hide();
        }
    });
});