...
$res=mysql_query("select * from r_tours_participants where user_id=$userid and position_id=1 and countedToRank=1");
$titles=mysql_num_rows($res);

$res=mysql_query("select * from r_tours_participants where user_id=$userid and (position_id=1 or position_id=2) and countedToRank=1");
...
if($current_userid==$userid){
	...
	$res=mysql_query("select sum(match_points_won) as total_points from r_calls where yearweek = '".date('oW',strtotime('-1 year'))."' and match_winner_user_id=$userid");
	$data['Week0'][0]="W".date('W',strtotime('-1 year'));
	$week_start= date('N',strtotime('-1 year'))==1 ? date('d-m-Y',strtotime('-1 year')) : date('d-m-Y',strtotime('last monday',strtotime('-1 year')));
	$week_end=   date('N',strtotime('-1 year'))==7 ? date('d-m-Y',strtotime('-1 year')) : date('d-m-Y',strtotime('next sunday',strtotime('-1 year')));
	if($row=mysql_fetch_object($res)){
	...
	$res=mysql_query("select sum(match_points_won) as total_points from r_calls where yearweek = '".date('oW',strtotime('-1 year +1 week'))."' and match_winner_user_id=$userid");
	$data['Week1'][0]="W".date('W',strtotime('-1 year +1 week'));
	$week_start= date('N',strtotime('-1 year +1 week'))==1 ? date('d-m-Y',strtotime('-1 year +1 week')) : date('d-m-Y',strtotime('last monday',strtotime('-1 year +1 week')));
	$week_end=   date('N',strtotime('-1 year +1 week'))==7 ? date('d-m-Y',strtotime('-1 year +1 week')) : date('d-m-Y',strtotime('next sunday',strtotime('-1 year +1 week')));
	if($row=mysql_fetch_object($res)){
	...
	$res=mysql_query("select sum(match_points_won) as total_points from r_calls where yearweek = '".date('oW',strtotime('-1 year  +2 week'))."' and match_winner_user_id=$userid");
	$data['Week2'][0]="W".date('W',strtotime('-1 year +2 weeks'));
	$week_start= date('N',strtotime('-1 year +2 week'))==1 ? date('d-m-Y',strtotime('-1 year +2 week')) : date('d-m-Y',strtotime('last monday',strtotime('-1 year +2 week')));
	$week_end=   date('N',strtotime('-1 year +2 week'))==7 ? date('d-m-Y',strtotime('-1 year +2 week')) : date('d-m-Y',strtotime('next sunday',strtotime('-1 year +2 week')));
	if($row=mysql_fetch_object($res)){
	...
	$res=mysql_query("select sum(match_points_won) as total_points from r_calls where yearweek = '".date('oW',strtotime('-1 year  +3 week'))."' and match_winner_user_id=$userid");
	$data['Week3'][0]="W".date('W',strtotime('-1 year +3 weeks'));
	$week_start= date('N',strtotime('-1 year +3 week'))==1 ? date('d-m-Y',strtotime('-1 year +3 week')) : date('d-m-Y',strtotime('last monday',strtotime('-1 year +3 week')));
	$week_end=   date('N',strtotime('-1 year +3 week'))==7 ? date('d-m-Y',strtotime('-1 year +3 week')) : date('d-m-Y',strtotime('next sunday',strtotime('-1 year +3 week')));
	if($row=mysql_fetch_object($res)){
...
$res=mysql_query("select * from r_calls where ((caller_user_id=$userid and callee_user_id=$current_userid) or (callee_user_id=$userid and caller_user_id=$current_userid)) and call_completed=1 order by match_date desc limit 5");
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
  ...
  <div style="min-width:300px;">
    ...
    <?php
		...
		if($days_to_bonus>0) echo $days_to_bonus_text;
		}
