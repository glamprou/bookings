...
abstract class Page implements IPage
{
	...
	protected function __construct($titleKey = '', $pageDepth = 0)
	{
		...
		$this->smarty->assign('Title', $resources->GetString('ApplicationName').' - ' . $resources->GetString($titleKey));
	...
	/**
	 * LOOKUP
	 * dimiourgei k etoimazei to kyriws periexomeno gia na ginei displayed
	 * @param string $templateName
	 */
	protected function GenerateContent($generatorName)
	{
            require_once(ROOT_DIR."GenerateContent/".$generatorName."_gen.php");
	}
	
	/**
	 * LOOKUP
	 * emfanizei to kyriws periexomeno, diladi oxi header i footer
	 * xrisimopoieitai sto ranking meros kai emfanizei ta dika mou templates se php 
	 * pou vriskontai sto tpl/Ranking/...
	 * @param string $templateName
	 */
	protected function DisplayContent($templateName)
	{
		require_once(ROOT_DIR."tpl/Ranking/".$templateName."_tpl.php");
	}
