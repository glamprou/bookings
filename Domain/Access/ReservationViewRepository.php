...
class ReservationViewRepository implements IReservationViewRepository
{
	...
	public function GetReservationList_excluded(
		Date $startDate,
		Date $endDate,
		$userId = self::ALL_USERS,
		$userLevel = ReservationUserLevel::OWNER,
	...
	public function GetList($pageNumber, $pageSize, $sortField = null, $sortDirection = null, $filter = null)
	{
		$command = new GetFullReservationListCommand(!empty($_GET['uid']));
	...
	{
		//eisagwgi tou 1ou participant na einai 1os sti seira	
		$result=pdoq("select * from reservation_first_participant where reservation_series_id = ? order by reservation_first_participant asc",$reservationView->SeriesId);
		if($result){
            foreach($result as $res){
                $res1=pdoq("SELECT user_id, fname, lname, email FROM users  WHERE user_id = ?",$res->user_id);
                $row1=$res1[0];

                $rows2=pdoq("SELECT reservation_user_level FROM reservation_users  WHERE reservation_instance_id=$reservationView->ReservationId and user_id = ?",$row1->user_id);
                $levelId = $rows2[0]->reservation_user_level;
		...
		while ($row = $result->GetRow())
		{
			...
			$reservationView->Participants=array_unique($reservationView->Participants);
		}
	}
...
class ReservationItemView implements IReservedItemView
{
	...
	)
	{
        ...
        $this->double_last_name = $userLastName;
		$rows=pdoq("select series_id from reservation_instances where reservation_instance_id=?",$reservationId);

		$rows=pdoq("select u.lname from users u inner join reservation_first_participant rfp on u.user_id=rfp.user_id where rfp.reservation_series_id=? order by reservation_first_participant_id asc limit 1",$rows[0]->series_id);
        if($rows){
            $this->double_last_name = $userLastName." ".$rows[0]->lname;
        }
        else{
            $rows=pdoq("select u.lname from users u inner join reservation_users ru on u.user_id=ru.user_id where ru.reservation_instance_id=? and ru.reservation_user_level = 2", $reservationId);
            if($rows){
                $this->double_last_name = $userLastName." ".$rows[0]->lname;
            }
        }
